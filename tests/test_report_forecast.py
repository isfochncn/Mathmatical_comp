...
def test_future_actuals_and_unreleased_reports_cannot_change_forecast(synthetic):
    ...
    b=ReportAwareForecaster(changed,tl2)
...
def test_dispatch_routes_only_problem_three_to_new_pv(synthetic):
    tl=build_timeline(synthetic)
    assert isinstance(make_forecaster(RunConfig(problem='problem3'),synthetic,tl),ReportAwareForecaster)
    for problem in ('problem2','problem4-2','problem4-3'):
        assert type(make_forecaster(RunConfig(problem=problem),synthetic,tl)) is AdaptiveForecaster
