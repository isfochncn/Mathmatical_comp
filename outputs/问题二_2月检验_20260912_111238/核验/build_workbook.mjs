...
// Use Excel serials to preserve source wall-clock times without local timezone
// normalization of JavaScript Date values at export boundaries.
const date = text => Date.parse(text.length === 10 ? `${text}T00:00:00Z` : `${text}Z`) / 86400000 + 25569;
...
setup(detail,'Q',end,'问题二：逐时购电与费用',`${monthLabel}归属行；普通购电日内冻结，外网总购电包含紧急补购。`);
...
setup(summary,'H',28,`问题二：${monthLabel}检验`,`本次从2025年1月1日00:10、6000 kWh连续运行至${payload.to.replace('T',' ')}。`);
...
summary.getRange('F18:H26').values=[
  ...
  ['旧版对照','旧版全年问题二轨迹','两版本均由年初运行；报告月初SOC可能不同'],
...
const file=path.join(output,`问题二_${monthLabel}_逐时购电与电费.xlsx`);
